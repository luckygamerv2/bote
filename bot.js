...
if (message.content === 'b!invite') {
      message.author.send(' رابط دعوه البوت https://discordapp.com/api/oauth2/authorize?client_id=548836729415794688&permissions=0&scope=bot
      
      الشخص اللي سوا البوت 
      luckygamer#0111
...
if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('ليس لديك صلاحيات');
 ...
 client.on('message', message=> {
    ...
    if (message.isMentioned(client.user))
    {
    message.reply(" ||صنع بواسطة @luckygamer#0111 || ");
    }
});
